/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     18/09/2018 13:01:54                          */
/*==============================================================*/
...
(
   ...
   ID_INI               int(12) not null comment 'Identificador unico en Tabla DATOS INICIAL',
...
(
   ...
   ID_INI               int(12) default NULL comment 'CONTIENE EL CODIGO UNICO DE IDENTIFICACION DE LA TABLA DATOS INICIAL',
...
(
   ...
   CODIGO_PROD          int(10) default NULL,
...
alter table afc_agricola add constraint FK_DAT_INI_AFC_AGRICOLA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_agricola_m2 add constraint FK_DAT_INI_AFC_AGRICOLA_M2 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_amenazasm12 add constraint FK_DAT_INI_AFC_AMENAZASM12 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_apicultura add constraint FK_DAT_INI_AFC_APICULTURA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_apicultura_m7 add constraint FK_DAT_INI_AFC_APICULTURA_M7 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_apicultura_mod7 add constraint FK_DAT_INI_AFC_APICULTURA_MOD7 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_credito_asistenciatec_m9 add constraint FK_DAT_INI_AFC_CRE_ASIS_TEC_M9 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_equipamientom11 add constraint FK_DAT_INI_AFC_EQUIPA_M11 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_gfc_mod1 add constraint FK_DAT_INI_AFC_GFC_MOD1 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;
...
alter table afc_gfc_mod1_2 add constraint FK_DAT_INI_AFC_GFC_MOD1_2 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_gfc_mod1_2 add constraint FK_PROD_AFC_MOD1_2 foreign key (CODIGO_PROD)
      references productor (CODIGO_PROD) on delete restrict on update restrict;

alter table afc_infraestructura_m10 add constraint FK_DAT_INI_AFC_INFRA_M10 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_ingresos_m13 add constraint FK_DAT_INI_AFC_INGRESOS_M13 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_pecuario add constraint FK_DAT_INI_AFC_PERCUARIO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_pecuario_m3 add constraint FK_DAT_INI_AFC_PECUARIO_M3 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_producto_elaborado add constraint FK_DAT_INI_AFC_PROD_ELABORADO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_producto_elaborado_m5 add constraint FK_DAT_INI_AFC_PROD_ELEBORA_M5 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_recoleccion add constraint FK_DAT_INI_AFC_RECOLEC foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_recoleccion_m6 add constraint FK_DAT_INI_AFC_RECOLEC_M6 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_tipo_produccion_mod4 add constraint FK_DAT_INI_AFC_TIPO_PROD_MOD4 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_trabajo_fam_con_mod8 add constraint FK_DAT_INI_AFC_TRAB_FAM_CON_MOD8 foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table afc_trabajo_fam_con_mod8 add constraint FK_PROD_AFC_TRA_FAM_CON_MOD8 foreign key (CODIGO_PROD)
      references productor (CODIGO_PROD) on delete restrict on update restrict;

alter table asistencia_tecnica add constraint FK_DAT_INI_ASIST_TECNICA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table base_reg_civil add constraint FK_PROD_BASE_REG_CIV foreign key (CODIGO_PROD)
      references productor (CODIGO_PROD) on delete restrict on update restrict;

alter table credito add constraint FK_DAT_INI_CREDITO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table cultivos add constraint FK_DAT_INI_CULTIVOS foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table datos_informante add constraint FK_DAT_INI_DAT_INFORMANTE foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table datos_productor add constraint FK_PROD_DATOS_PROD foreign key (CODIGO_PROD)
      references productor (CODIGO_PROD) on delete restrict on update restrict;

alter table destino_produccion add constraint FK_DAT_INI_DEST_PRODUCCION foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_existencias_plantel add constraint FK_DAT_INI_ESP_AVI_EXIST_PLANTEL foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_galpones add constraint FK_DAT_INI_ESP_AVI_GALPON foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_plantel add constraint FK_DAT_INI_ESP_AVI_PLAN foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_plantel_ciclos add constraint FK_DAT_ESP_AVI_PLA_CIC foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_plantel_encubadoras add constraint FK_DAT_INI_ESP_AVI_PLA_ENC foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_avicola_traspatio add constraint FK_DAT_INI_ESP_AVI_TRASPATIO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_bovino add constraint FK_DAT_INI_ESP_BOVINO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_bovino_leche add constraint FK_DAT_INI_ESP_BOV_LECHE foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_bovino_ventas add constraint FK_DAT_INI_ESP_BOV_VENTAS foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_otros add constraint FK_DAT_INI_ESP_OTROS foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_porcino add constraint FK_DAT_INI_ESP_PORCINO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_porcino_enfermedades add constraint FK_DAT_INI_ESP_POR_ENF foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_porcino_galpones add constraint FK_DAT_INI_ESP_POR_GAL foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table especie_porcino_reproduccion add constraint FK_DAT_INI_ESP_POR_REP foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table formulario_verificar add constraint FK_DAT_INI_FOR_VERIFICAR foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table formulario_verificar add constraint FK_PROD_FOR_VERIFICAR foreign key (CODIGO_PROD)
      references productor (CODIGO_PROD) on delete restrict on update restrict;

alter table gps_terrenos add constraint FK_DAT_INI_GPS_TERRENOS foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;
...
alter table infraestructura add constraint FK_DAT_INI_INFRAESTRUCTURA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table infraestructura_riego add constraint FK_DAT_INI_INFRAESTRUCTURA_RIEGO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table log_auditoria add constraint FK_USUARIO_LOG_AUDITORIA foreign key (CODIGO_USR)
      references usuarios (CODIGO_USR) on delete restrict on update restrict;

alter table log_transaccion add constraint FK_DAT_INI_LOG_TRAN foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table mano_obra add constraint FK_DAT_INI_MAN_OBR foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table mano_obra_ingresos add constraint FK_DAT_INI_MAN_OBR_INGRESO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table mano_obra_jornaleros add constraint FK_DAT_INI_MAN_OBR_JORNAL foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table maquinaria add constraint FK_DAT_INI_MAQUINARIA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table medidas_sanitarias add constraint FK_DAT_INI_MED_SANITARIA foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;
...
alter table produccion_forestal add constraint FK_DAT_INI_PROD_FORESTAL foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table productor add constraint FK_DAT_INI_PRODUCTOR foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;
...
alter table terrenos add constraint FK_DAT_INI_TERRENOS foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;

alter table uso_suelo add constraint FK_DAT_INI_USO_SUELO foreign key (ID_INI)
      references datos_inicial (ID_INI) on delete restrict on update restrict;
